...
module.exports = {
  regisUserUser: async (userData) => {
    ...
    try {
      ...
      const checkedEmail = await db.get().collection(collection.USER_COLLECTION).findOne({ email })
      const phoneNumber = await db.get().collection(collection.USER_COLLECTION).findOne({ mobile })
      if (checkedEmail) {
        return checkedEmail
      } else if (phoneNumber) {
        return phoneNumber
  ...
  loginUser: async (userData) => {
    ...
    try {
      const response = {}
      const user = await db.get().collection(collection.USER_COLLECTION).findOne({ mobile })
      if (!user?.active) {
        response.block = true
        return response
      }
  ...
  editAddress: async (addressId, addressInfo) => {
    try {
      const { name, mobile, pincode, locality, address, city, state, landmark, alternatePhone } = addressInfo
      const response = await db.get().collection(collection.ADDRESS_COLLECTION).updateOne(
        { _id: ObjectId(addressId) },
        { $set: { name, mobile, pincode, locality, address, city, state, landmark, alternatePhone } }
      )
      return response
    } catch (error) {
      console.log(error)
  ...
  placeOrders: async (orderInfo, products, totalPrice) => {
    try {
      const { userId, payment_method: paymentMethod, name, mobile, deliveryAddress } = orderInfo
      ...
      const resp = await db.get().collection(collection.ORDER_COLLECTION).insertOne(order)
      await db.get().collection(collection.CART_COLLECTION).deleteOne({ userId: ObjectId(orderInfo.userId) })
      return resp
    } catch (error) {
      console.log(error)
  ...
  createStatusCollection: (orderId) => {
    try {
      return new Promise((resolve, reject) => {
        ...
        db.get().collection(collection.ORDER_SATUS).insertOne(status).then((response) => {
          resolve(response)
        })
      })
    } catch (error) {
  ...
  getStatusDates: async (orderId) => {
    try {
      const dates = await db.get().collection(collection.ORDER_SATUS).findOne({ orderId })
      return dates
    } catch (error) {
      console.log(error)
      throw new Error(error)
  ...
  createGuestUser: async (guestId, productId) => {
    ...
    try {
      ...
      const guestCart = await db.get().collection(collection.GUEST_USERS).findOne({ guestId })
      ...
      if (guestCart) {
        ...
        if (isProductExist !== -1) {
          await db.get().collection(collection.GUEST_USERS)
            .updateOne({ guestId, 'products.item': ObjectId(productId) },
              {
                $inc: { 'products.$.quantity': 1 },
          ...
          await db.get().collection(collection.GUEST_USERS).updateOne({ guestId }, {
            $push: {
              products: product
            },
            $set: { updatedAt: dateTimeString }
        ...
        await db.get().collection(collection.GUEST_USERS).insertOne(guestDocument)
      }
      return true
    } catch (error) {
      console.log(error)
  ...
  getGuestUserCartProducts: async (guestId) => {
    try {
      const cartItems = await db.get().collection(collection.GUEST_USERS).aggregate([
        {
          $match: { guestId }
        },
        {
  ...
  mergeGuestCartIntoUserCart: async (userId, guestId) => {
    ...
    const guestUser = await db.get().collection(collection.GUEST_USERS).findOne({ guestId })
    console.log(guestUser)
    const result = await db.get().collection(collection.CART_COLLECTION).findOneAndUpdate(
      { userId: ObjectId(userId), 'products.item': { $in: guestUser.products.map(p => p.item) } },
      {
        $inc: {
